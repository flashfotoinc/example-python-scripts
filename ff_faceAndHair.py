...
segment_image_url = '%sapi/segment/%s%s' % (options.base_url, image_id, auth_params)

#send the command
print "\nRemoving Background:"
try:
	perform_segment = urllib2.urlopen(segment_image_url)
	response = perform_segment.read()
except urllib2.HTTPError, e:
	print e
	print segment_image_url
	sys.exit(1)
...
#send the command
try:
	perform_segment_status = urllib2.urlopen(segment_status_url)
	response = perform_segment_status.read()
except urllib2.HTTPError, e:
	print e
	print segment_status_url
	sys.exit(1)
#self closes so no need to call perform_segment.close()
...
try:
        response = json.loads(response)
	while (response['segmentation_status'] != 'finished' and response['segmentation_status'] != 'failed'):
	    time.sleep(4)
	    print json.dumps(response)
	    perform_segment_status = urllib2.urlopen(segment_status_url)
	    response = perform_segment_status.read()
	    response = json.loads(response)
	if (response['segmentation_status'] == 'finished'):
	    print json.dumps(response)
except ValueError, e:
	print e
	print 'Response: %s' % response
...
if options.output_mask:
	...
	mask_url = "%sapi/get/%s%s%s" % (options.base_url, image_id, auth_params, '&version=HardMask')
...
if options.output_masked:
	#we do, let's build the url
	masked_url = "%sapi/get/%s%s%s" % (options.base_url, image_id, auth_params, '&version=HardMasked')
